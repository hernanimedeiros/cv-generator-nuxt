<template>
  <section class="static-item wrapper p-2 w-100">
    <b-row>
      <!-- Sidebar -->
      <b-col
...
export default {
  ...
  computed: {
    ...
    // Link modifiers
    mailLink () {
      const email = String(this.getProfile.profileEmail).replace(/\s+/g, '')
      const emailPrefix = 'mailto:'
      const emailSubject = '?subject=C.V.%msg:'
      return emailPrefix + email + emailSubject
    },
    phoneLink () {
      const phone = String(this.getProfile.profileEmail).replace(/\s+/g, '')
